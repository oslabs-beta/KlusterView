...
const portForwardingOperational = (
  ...
  for (const [service, port] of servicePortPairs) {
    ...
    if (service && String(port).length >= 4) {
      if (service.search('prom') !== -1 && port.slice(0, 4) === '9999') {
        prom9999 = true;
      }
      if (service.search('grafana') !== -1 && port.slice(0, 4) === '9000') {
        graf9000 = true;
      }
    }
  }
  console.log(`Prom9999: ${prom9999}, Graf9000:${graf9000}`);
